...
class Name:
    ...
    def writing_sentence(self, sentence, args):
        fobj_out = open("corpus_files"+"/"+args.file, "a")
    ...
    def check_file(self, args, lang):
        ...
        # self.poodle_loader(lang, args)
        # self.voice_web_loader(lang, args)
        fobj_in = open("corpus_files"+"/"+args.file)
        fobj_out = open("corpus_files"+"/"+"corpus_"+args.file, "w+")
        i = 1
        for line in fobj_in:
            line = line.replace('\n', '')
            line = line.replace('  ', ' ')
            ...
            write_out = str(line)+"\t"+str(len(line))+"\n"
            #d = 1
            #if os.path.isfile("prompts"+"/"+"corpus_"+args.file):
            if write_out not in fobj_out: # checks if entry already exists
                fobj_out.write(str(write_out))
            #else:
        ...
        fobj_out.close()
        #request = ""
        #csv_contents = request.vars["prompts"+"/"+"corpus_"+args.file].value.decode("utf-8")
       # file = csv_contents.splitlines()
        # Duplicate entries are removing
        #file_data = list(set([row for row in csv.DictReader("prompts"+"/"+"corpus_"+args.file)]))
